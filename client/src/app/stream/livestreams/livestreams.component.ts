...
export class LivestreamsComponent implements OnInit , OnChanges, AfterContentChecked {
  ...
  getLiveStreams() {
    axios.get('http://34.70.181.173:' + config.rtmp_server.http.port + '/api/streams')
        .then(res => {
  ...
  getStreamsInfo(live_streams) {
    axios.get('http://104.154.246.10:3333/streams/info', {
      // axios.get('http://34.70.181.173:' + config.rtmp_server.http.port  + '/streams', {
