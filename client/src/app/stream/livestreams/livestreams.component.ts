...
export class LivestreamsComponent implements OnInit , OnChanges, AfterContentChecked {
  ...
  getLiveStreams() {
    axios.get('http://35.192.161.182:' + config.rtmp_server.http.port + '/api/streams')
        .then(res => {
  ...
  getStreamsInfo(live_streams) {
    axios.get('http://35.192.161.182:3333/streams/info', {
      // axios.get('http://35.192.161.182:' + config.rtmp_server.http.port  + '/streams', {
